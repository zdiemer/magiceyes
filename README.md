...
As of the 13 August 2026 sweep of a 1,031-title corpus (all three handhelds), **557 titles
are fully playable** and 595 put real gameplay on screen; 170 of the failures are broken or
incomplete dumps rather than emulator gaps. The full per-title scoreboard is regenerated by
`tools/test/` (see `tools/test/CORPUS_SWEEP.md`).
